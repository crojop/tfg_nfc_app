 ...
 */
public class CombiLineChart extends AbstractChart {
	...
	public Intent execute(Context context) {
		...
		String[] titles = new String[] { "Class AVERAGE time", "YOUR time", "ESTIMATED time"  };

		List<double[]> values = new ArrayList<double[]>();

		int[] colors = new int[] { Color.parseColor("#D6021B"),
				Color.parseColor("#6B0480"), Color.parseColor("#003194") };
		...
		for (int i = 0; i < las.size(); i++) {
			...
			Log.d(CLASSNAME, i + " / " + sXLabel + " / estimated "
					+ adAVGClassroom[i] + " / accomplished " + adAccomplished[i] + " / estimated " + adEstimated[i]);
		...
		setChartSettings(renderer,
				...
				"Number of hours", 0, adAVGClassroom.length / 3, 0, 10,
		...
		renderer.getSeriesRendererAt(2).setChartValuesTextSize(iSizeFont);		
